...
function getBreakfast() {
  ...
  request.onload = function () {
    if (request.status >= 200 && request.status < 400) {
      ...
      document.getElementById("breakfast-name").innerText =
        data[randomMeal].name;
...
function showBreakfastInstructions() {
  document
    .getElementById("breakfast-click-instructions")
...
function showLunchInstructions() {
  document
    .getElementById("lunch-click-instructions")
    .classList.remove("hidden");
}
...
function showDinnerInstructions() {
  document
    .getElementById("dinner-click-instructions")
    .classList.remove("hidden");
}
...
let searchingData = async (e) => {
  ...
  for (let i = 0; i < Data.length; i++) {
    if (
      SearchedData === Data[i].name.toLowerCase() ||
      ...
      document
        .querySelector("#search-click-instructions")
